...
describe('reducer builder', () => {
  ...
  it('should execute the default handler if no cases provided', () => {
    const reducer = reducerWithoutInitialState<State>().default(defaultHandler);
  ...
  (() => {
    ...
    it('should call handler on any matching action when using .cases()', () => {
      ...
      expect(reducer(initialState, actionA({ data: 'from A', x: 0 }))).toEqual({
        data: 'from A',
      });
      expect(reducer(initialState, actionB({ data: 'from B', y: 1 }))).toEqual({
        data: 'from B',
      });
      expect(reducer(initialState, actionC({ data: 'from C', z: 2 }))).toEqual({
        data: 'from C',
      });
    });

    it('should call handler on any matching action when using .casesWithAction()', () => {
      const reducer = reducerWithInitialState(initialState).casesWithAction(
        [actionA, actionB, actionC],
        (state, action) => {
  ...
  it('should apply handling function to itself in .withHandling()', () => {
    const handling = (builder: ReducerBuilder<State>): ReducerBuilder<State> =>
